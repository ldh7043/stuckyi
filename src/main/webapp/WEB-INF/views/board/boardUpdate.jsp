...
<section id="boardWrite" class="boardWrite">
	<h2 class="boardTitle">수정</h2>
	<div class="bwCon">
		<sec:authorize access="isAuthenticated()">
			<sec:authentication property="principal.username" var="user_id" />
		</sec:authorize>
		<form name="frm" action="updateOk" method="post" onsubmit="return chkSubmit()">
			<input type="hidden" name="brdUid" value="${list[0].brdUid}"/>
			작성자: ${user_id}<br><br>
			제목:
			<input class='btn btn-outline-info' type="text" name="brdTitle" value="${list[0].brdTitle}"/><br><br>
			타입:
			...
			내용:<br>
			<textarea class='btn btn-outline-info' rows="5" cols="50" name="brdContent" >${list[0].brdContent}</textarea>
			<br><br>
			<input class='btn btn-outline-info' type="submit" value="등록"/>
			<input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }" />
		</form>
		<br>
