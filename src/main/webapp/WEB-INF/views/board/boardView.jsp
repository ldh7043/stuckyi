...
<body>
   ...
   <header id="header">
	...
	<h2 class="boardTitle">게시글</h2>
    <div class="commentContainer">
	<!-- 게시판 보기 -->
	<form id="commentFrm">
		<button type="button" class='btn btn-outline-info' onclick="location.href = 'http://localhost:8033/stuckyi/board/list'">목록으로 돌아가기</button><br><br>
		<table class="boardView">
			<tr>
				<td>작성자</td>
				<td>${boarditem[0].userId }</td>
			</tr>
			<tr>
				<td>제목</td>
				<td>${boarditem[0].brdTitle }</td>
			</tr>
			<tr>
				<td>등록일</td>
				<td>${boarditem[0].brdRegdate }</td>
			</tr>
			<tr>
				<td>조회수</td>
				<td>${boarditem[0].brdViewcnt }</td>
			</tr>
			<tr>
				<td>내용</td>
				<td class="brdContent">${boarditem[0].brdContent }</td>
			</tr>
		</table>

		<sec:authorize access="isAuthenticated()">
           	<sec:authentication property="principal.username" var="user_id" />
			<c:if test="${user_id == boarditem[0].userId}">	
				<div>
					<button type="button" class='btn btn-outline-info' class="button" onclick="location.href='update?uid=${boarditem[0].brdUid }'">수정</button>
					<button type="button" class='btn btn-outline-info' class="button" onclick="location.href='delete?uid=${boarditem[0].brdUid }'">삭제</button>
				</div>
			</c:if>
			<!-- 댓글 작성 -->
			<input type="hidden" id="userid" name="userid" value="${user_id }"/><br>
			<span class="name">아이디: ${ user_id }</span><br>
			<div class="clear"></div>
			<input type="hidden" id="brdUid" value="${boarditem[0].brdUid }"/><br>

			<c:if test="${user_id != null}">
			 <textarea class='btn btn-outline-info' rows="5" cols="50" id="cmtContent"
	             placeholder="댓글을 작성하세요"></textarea><br><br>

				<button type="button" class='btn btn-outline-info' class="button" id="btnReply">댓글쓰기</button>
			</c:if>
		</sec:authorize>
		<input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }" />
	...
	<div id="commentList">
		<form name="cfrm">
			<input type="hidden" id ="brdUid" name="brdUid" value="${boarditem[0].brdUid }"><br>
			<input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token }" />
		</form>
		
		<h2 class="commentList">댓글 목록</h2>
	    <table id="commentJSON"></table>
...
<!-- ======= Footer ======= -->
  ...
  <script src="<%= request.getContextPath() %>/resources/assets/js/cmt.js"></script>
