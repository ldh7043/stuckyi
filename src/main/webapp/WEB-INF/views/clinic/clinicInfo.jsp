...
<div class="row">
   <div class="col-sm-7" id="map" style="width:100%;height:500px;"></div>
   <div class="col-sm-5"><p id="hosshow"></p></div>
</div>
<script>
   var api_key = "BtBjxIB2BPCUhsmt0Obsj20HugxlL5tE0LA7MFrtt519Nyz00rSsabKEGX6vRG%2Ft%2FH2zKLCa3miqYvCzv%2BqPew%3D%3D";
    ...
    $(document).ready(function(){
       // XML
        var url = "http://apis.data.go.kr/B551182/hospInfoService/getHospBasisList?yadmNm=<%=(String)request.getParameter("yadmNm") %>&ServiceKey=" + api_key;
        console.log(url);
    ...
    function parseXML(xmlDOM) {
        ...
        $(xmlDOM).find("item").each(function(){
            setMapMarker($(this).find("XPos").text(), $(this).find("YPos").text());
            document.getElementById("hosshow").innerHTML = 
               "병원이름 : " + $(this).find("yadmNm").text() +
               "<br> 전화번호 : " + $(this).find("telno").text() + 
               "<br> 우편번호 : " + $(this).find("postNo").text() + 
               "<br> 병원주소 : " + $(this).find("addr").text() + 
               "<br> 병원종별 : " + $(this).find("clCdNm").text() + 
               "<br> 의사 수 : " + $(this).find("drTotCnt").text() +
               "<br> 전문의 수 : " + $(this).find("sdrCnt").text() +
               "<br> 일반의 수 : " + $(this).find("gdrCnt").text() + 
               "<br> 레지던스 수 : " + $(this).find("resdntCnt").text() + 
               "<br> 홈페이지 : " + "<a href=" + $(this).find('hospUrl').text() + ">" + $(this).find('hospUrl').text() + "</a>";
