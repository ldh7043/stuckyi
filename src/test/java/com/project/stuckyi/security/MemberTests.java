...
public class MemberTests {
	...
	final String SQL_INSERT_MEMBER = "INSERT INTO join(user_id, user_pw, user_name, user_email) VALUES (?,?,?,?)";
	final String SQL_INSERT_AUTH = "INSERT INTO admin_auth (user_id, user_auth) VALUES (?,?)";
...
if(conn == null) return;
		...
		try {
			...
			for(int i = 0; i < 100; i++) {
				...
				if(cnt > 0) {
					System.out.println("INSERT_MEMBER 성공]" + userid + ":" + userpw + ":" + username + ":" + useremail);
				} else {
					System.out.println("INSERT_MEMBER 실패]" + userid + ":" + userpw + ":" + username + ":" + useremail);
				}
